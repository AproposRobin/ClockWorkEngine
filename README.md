...
1. **Composition-first entities.** Entities are built by chaining together components rather than relying on deep inheritance hierarchies. The engine is architected around ECS (Entity Component System) principles so behavior can be assembled, reused, and swapped at the component level.
...
| `ClockWorkUI` | UI layer shared between the engine and editor |
